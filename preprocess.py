...
def get_affinity_graph(dataset, adj, num_pos, pos_threshold, scenario='S1'):
    ...
    # 安全提取 drug_pos
    if torch.is_tensor(drug_pos):
        if drug_pos.is_sparse:
            d_adj = drug_pos.to_dense().cpu().numpy()
        else:
            d_adj = drug_pos.cpu().numpy()
    elif sp.issparse(drug_pos):
        d_adj = drug_pos.toarray()
    else:
        d_adj = np.array(drug_pos)
        
    # 安全提取 target_pos
    if torch.is_tensor(target_pos):
        if target_pos.is_sparse:
            t_adj = target_pos.to_dense().cpu().numpy()
        else:
            t_adj = target_pos.cpu().numpy()
    elif sp.issparse(target_pos):
        t_adj = target_pos.toarray()
    else:
        t_adj = np.array(target_pos)
    # ====================================================

    # 真正构建包含4种边的全图邻接矩阵
    adj = np.concatenate((
        np.concatenate((d_adj, adj_1), 1),
        np.concatenate((adj_2, t_adj), 1)
    ), 0)
