...
class TrueHeteroGATModel(nn.Module):
    def __init__(self, hidden_dim, out_dim, heads=1, dropout_rate=0.2):
        super(TrueHeteroGATModel, self).__init__()
        
        # 严格对应论文：定义四种关系的异构图注意力卷积
        # 注意：异构图中的双边关系必须设置 add_self_loops=False 避免维度报错
        self.conv1 = HeteroConv({
            ('drug', 'sim', 'drug'): GATConv(hidden_dim, hidden_dim, heads=heads, add_self_loops=False),
            ('drug', 'interact', 'target'): GATConv(hidden_dim, hidden_dim, heads=heads, add_self_loops=False),
            ('target', 'interact', 'drug'): GATConv(hidden_dim, hidden_dim, heads=heads, add_self_loops=False),
            ('target', 'sim', 'target'): GATConv(hidden_dim, hidden_dim, heads=heads, add_self_loops=False),
        }, aggr='sum')
        
        self.conv2 = HeteroConv({
            ('drug', 'sim', 'drug'): GATConv(hidden_dim, out_dim, heads=heads, add_self_loops=False),
            ('drug', 'interact', 'target'): GATConv(hidden_dim, out_dim, heads=heads, add_self_loops=False),
            ('target', 'interact', 'drug'): GATConv(hidden_dim, out_dim, heads=heads, add_self_loops=False),
            ('target', 'sim', 'target'): GATConv(hidden_dim, out_dim, heads=heads, add_self_loops=False),
        }, aggr='sum')
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        x_dict = {key: self.dropout(self.relu(x)) for key, x in x_dict.items()}
        x_dict = self.conv2(x_dict, edge_index_dict)
        return x_dict

class HeteroAffinityGAT(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, heads=1, dropout_rate=0.2):
        super(HeteroAffinityGAT, self).__init__()
        # 初始特征映射
        self.lin_drug = nn.Linear(in_dim, hidden_dim)
        self.lin_target = nn.Linear(in_dim, hidden_dim)
        # 异构网络本体
        self.hetero_gat = TrueHeteroGATModel(hidden_dim, out_dim, heads, dropout_rate)
        
    def forward(self, graph, num_drug, num_target):
        xs = graph.x
        adj = graph.adj
        
        # 1. 从大邻接矩阵中拆分出4种边的子矩阵
        adj_dd = adj[:num_drug, :num_drug]
        adj_dt = adj[:num_drug, num_drug:]
        adj_td = adj[num_drug:, :num_drug]
        adj_tt = adj[num_drug:, num_drug:]
        
        # 2. 构建符合 PyG 规范的异构图字典
        edge_index_dict = {
            ('drug', 'sim', 'drug'): torch.nonzero(adj_dd, as_tuple=False).t(),
            ('drug', 'interact', 'target'): torch.nonzero(adj_dt, as_tuple=False).t(),
            ('target', 'interact', 'drug'): torch.nonzero(adj_td, as_tuple=False).t(),
            ('target', 'sim', 'target'): torch.nonzero(adj_tt, as_tuple=False).t()
        }
        
        # 3. 提取节点特征字典
        x_dict = {
            'drug': self.lin_drug(xs[:num_drug]),
            'target': self.lin_target(xs[num_drug:])
        }
        ...
        # 5. 拼接返回，对齐原代码的数据接口
        out = torch.cat([out_dict['drug'], out_dict['target']], dim=0)
        return [out]  # 包成列表是为了适配 [-1] 索引
...
class Contrast(nn.Module):
    def __init__(self, hidden_dim, output_dim, tau, lam):
        ...
        # 🚨 修复：药物和靶标的投影层，统一只接收 hidden_dim (256 维)
    ...
    def enhance_embedding(self, embedding, is_target=False):
        ...
        if embedding.size(1) == proj[0].in_features: 
            enhanced = proj(embedding)
            enhanced = F.normalize(enhanced, p=2, dim=1)
            return enhanced
        elif embedding.size(1) == proj[-1].out_features:
    ...
    def forward(self, za, zb, pos, is_target=False):
        # 此时送进来的 za 和 zb 已经全是 256 维了，完美通过！
        if is_target:
...
class UEC2DTA(nn.Module):
    def __init__(self, tau, lam, ns_dims, d_ms_dims, t_ms_dims, embedding_dim=128, heads=1, dropout_rate=0.2,
                 use_unimol=False, drug_hidden_dim=512, target_hidden_dim=1280, use_hetero=True):
        ...
        # 1. 亲和力图特征提取 (输出 256 维)
        self.affinity_graph_conv = HeteroAffinityGAT(
            in_dim=ns_dims[0], hidden_dim=ns_dims[1], out_dim=ns_dims[2], 
        ...
        if self.use_unimol:
            self.drug_feature_proj = nn.Sequential(
                nn.Linear(self.drug_hidden_dim, ns_dims[-1]),
                nn.ReLU(),
        ...
        # 3. 靶标特征降维层 (把 ESM2 的 1280 降维到 256)
        self.target_graph_conv = GATModel(t_ms_dims, heads=heads)
        self.target_feature_proj = nn.Sequential(
            nn.Linear(self.target_hidden_dim, ns_dims[-1]),
            nn.ReLU(),
            nn.Dropout(dropout_rate)
        )
        
        # 4. 对比学习模块 (统一接收干净的 256 维)
    ...
    def forward(self, affinity_graph, drug_graph_batchs, target_graph_batchs, drug_pos, target_pos, scenario='S1', enhance_embeddings=True):
        ...
        affinity_graph_embedding = self.affinity_graph_conv(affinity_graph, num_d, num_t)[-1]
        ...
        if self.use_unimol:
            if drug_graph_batchs.size(-1) != self.drug_hidden_dim:
                if drug_graph_batchs.size(-1) > self.drug_hidden_dim:
                    ...
                    padding = torch.zeros((drug_graph_batchs.size(0), self.drug_hidden_dim - drug_graph_batchs.size(-1)), device=drug_graph_batchs.device)
        ...
        if torch.sum(affinity_graph.adj[:num_d]) == 0:  
            valid_drug_indices = torch.where(torch.sum(affinity_graph.adj[:num_d], dim=1) != 0)[0]
        
        valid_affinity_drug_embedding = affinity_graph_embedding[valid_drug_indices]  # 256
        valid_drug_graph_embedding = drug_graph_embedding[valid_drug_indices]         # 256

        dru_loss, valid_drug_embedding = self.drug_contrast(valid_affinity_drug_embedding, valid_drug_graph_embedding, drug_pos, is_target=False)
        ...
        if valid_target_graph_batchs.size(-1) != self.target_hidden_dim:
            if valid_target_graph_batchs.size(-1) > self.target_hidden_dim:
                ...
                padding = torch.zeros((valid_target_graph_batchs.size(0), self.target_hidden_dim - valid_target_graph_batchs.size(-1)), device=valid_target_graph_batchs.device)
                valid_target_graph_batchs = torch.cat([valid_target_graph_batchs, padding], dim=1)
        
        valid_target_graph_batchs_proj = self.target_feature_proj(valid_target_graph_batchs) # 1280 -> 256 

        # 4. 对比学习 (256 vs 256)
        tar_loss, valid_target_embedding = self.target_contrast(
            target_affinity_embedding, 
            valid_target_graph_batchs_proj, 
            target_pos, 
            is_target=True
        )
